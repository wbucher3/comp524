...
;; ADDITIONS MADE FOR A6 AHHHH
;; We need to tell if a define or let keyword is here

;;treats the string as a name as normal
(define (name-token key)
  (token 'NAME (string->symbol key))
)

;; checks to see if it's one of the new scope words
(define (keyword? key)
  (case key
    (("define") (token "DEFINE"))
    (("lambda") (token "LAMBDA"))
